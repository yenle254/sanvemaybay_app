...
class _FillCustomerInfoPageSupportState
  ...
  //       "ob_aircode": airCode,
  //       "ob_base_price": basePrie.toString(),
  ...
  Future<Null> _getBagages(String domain, String aircode1, String aircode2) async {
    // Giả lập thời gian chờ tải dữ liệu hành lý
    await Future.delayed(Duration(milliseconds: 300));

    // 1. Giả lập Hành lý chiều đi
    listPackage = ["0kg", "15kg", "20kg", "25kg", "30kg"];
    mapPackage = {"0": "0kg", "15": "15kg", "20": "20kg", "25": "25kg", "30": "30kg"};
    mapPackagePrice = {"0": 0, "15": 150000, "20": 200000, "25": 250000, "30": 350000};
    mapPackageBuyPrice = {"0": 0, "15": 140000, "20": 180000, "25": 230000, "30": 330000};

    // Mặc định khách không mua hành lý (0kg)
    for (int i = 0; i < widget.flightInfo.listAdults.length; i++) {
    ...
    // 2. Giả lập Hành lý chiều về (nếu có khứ hồi)
    if (widget.flightInfo.isRoundTrip) {
      listPackage2 = ["0kg", "15kg", "20kg", "25kg", "30kg"];
      mapPackage2 = {"0": "0kg", "15": "15kg", "20": "20kg", "25": "25kg", "30": "30kg"};
      mapPackagePrice2 = {"0": 0, "15": 150000, "20": 200000, "25": 250000, "30": 350000};
      mapPackageBuyPrice2 = {"0": 0, "15": 140000, "20": 180000, "25": 230000, "30": 330000};
  ...
  Future<Map<String, dynamic>> _calculateFares({
    ...
    // Giả lập thời gian chờ tính thuế phí
    await Future.delayed(Duration(milliseconds: 300));

    // Giả lập mức thuế phí mặc định cho các đối tượng
    return {
      "adt_tax_fee": 450000, // Thuế người lớn
      "chd_tax_fee": 350000, // Thuế trẻ em
      "inf_tax_fee": 100000, // Thuế em bé
      "chd_base_price": (basePrie * 0.8).toInt(), // Giá vé trẻ em bằng 80% người lớn
      "inf_base_price": 0, // Em bé miễn phí vé, chỉ tính thuế
    };
  }
  ...
  void initState() {
    ...
    _calculateFares(
            airCode: widget.flightInfo.company1,
            basePrie: widget.flightInfo.priceDepart,
            noOfAdult: widget.flightInfo.noOfAdult,
            noOfChild: widget.flightInfo.noOfChild,
            noOfInfant: widget.flightInfo.noOfInfant)
        .then((fares) {
      setState(() {
        widget.flightInfo.adultDepartTax = fares["adt_tax_fee"];
        widget.flightInfo.childDepartTax = fares["chd_tax_fee"];
        widget.flightInfo.infantDepartTax = fares["inf_tax_fee"];
        widget.flightInfo.childDepartPrice = fares["chd_base_price"];
        widget.flightInfo.infantDepartPrice = fares["inf_base_price"];
    ...
    if (widget.flightInfo.isRoundTrip) {
      _calculateFares(
              airCode: widget.flightInfo.company2,
              basePrie: widget.flightInfo.priceBack,
              noOfAdult: widget.flightInfo.noOfAdult,
              noOfChild: widget.flightInfo.noOfChild,
              noOfInfant: widget.flightInfo.noOfInfant)
          .then((fares) {
        setState(() {
          widget.flightInfo.adultBackTax = fares["adt_tax_fee"];
          widget.flightInfo.childBackTax = fares["chd_tax_fee"];
          widget.flightInfo.infantBackTax = fares["inf_tax_fee"];
          widget.flightInfo.childBackPrice = fares["chd_base_price"];
          widget.flightInfo.infantBackPrice = fares["inf_base_price"];
    ...
    return new MaterialApp(
      ...
      home: new Scaffold(
        ...
        bottomNavigationBar: new TextButton(
          ...
          //               // Lưu thông tin liên hệ để người dùng sửa/hủy
          ...
          //             prefs.setStringList('my_tickets', savedData).then((_) {
          //               // Lưu xong thì bay thẳng sang trang Lịch sử (BookingHistoryPage)
                ...
                if (widget.flightInfo.contact.email.length > 0 &&
                  ...
                  if (!widget.flightInfo.contact.isValidPhone()) {
                    ...
                    try {
                      ...
                      print("=== 2. DỮ LIỆU VÉ: $newTicket ===");
                      savedData.add(json.encode(newTicket));

                      bool isSaved = await prefs.setStringList('my_tickets', savedData);
                      print("=== 3. TRẠNG THÁI LƯU VÀO MÁY: $isSaved ===");

                      if (isSaved) {
                        print("=== 4. CHUYỂN TRANG! ===");
                        Navigator.of(context).pushReplacement(new MaterialPageRoute(
                            builder: (context) => BookingHistoryPage()));
                      }
                    } catch (e) {
                      print("=== PHÁT HIỆN LỖI CRASH RỒI: $e ===");
                    }
                    // KẾT THÚC LƯU VÉ
                  }
                }
